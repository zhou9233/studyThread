...
public class UnsafCASTest {

    public static void main(String[] args) throws Exception {
        //通过反射实例化Unsafe
        Field f = Unsafe.class.getDeclaredField("theUnsafe");
        f.setAccessible(true);//作用就是让我们在用反射时访问私有变量
        ...
        int ageOffset = 12; // 修改内存偏移地址为12的值（age）,返回true,说明通过内存偏移地址修改age的值成功
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                System.out.println(unsafe.compareAndSwapInt(player, ageOffset, 12, 33));
