...
public abstract class AbstractQueuedSynchronizer
     ...
     * 重新设置头节点，并将新的头节点的线程2启动，这个过程执行的线程是上一个头节点的线程1，用线程1来unpark线程2
     * @param node the node
    ...
    private static boolean shouldParkAfterFailedAcquire(AbstractQueuedSynchronizer.Node pred, AbstractQueuedSynchronizer.Node node) {
        ...
        //如果前驱节点的等待状态 == SIGNAL -1 表示后继节点处于等待状态，那么后继节点node可以安全的被停放
        if (ws == AbstractQueuedSynchronizer.Node.SIGNAL)
            /*
        ...
        if (ws > 0) {
            /*
                如果前驱节点的等待状态大于0表示被取消，将该前驱节点从同步队列删除
    ...
    final boolean acquireQueued(final AbstractQueuedSynchronizer.Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                final AbstractQueuedSynchronizer.Node prev = node.predecessor();
                ...
                /**shouldParkAfterFailedAcquire如果prev的等待状态==-1，则node可以被安全的停放，并返回true
                如果prev的等待状态<=0 则将prev的等待状态设置为-1，并返回false
                如果prev的等待状态>0，则将prev从同步队列中移除，并返回false*/
                /**
                 * parkAndCheckInterrupt 确保停放该线程，并中断该线程
                 */
    ...
    private void doAcquireShared(int arg) {
        //addWaiter创建节点并将当前节点加入同步队列，当前节点包含当前线程，nextWaiter就是等待队列，如果当前节点是共享的，
        //那么nextWaiter将是一个SHARED常量，也就是说节点类型和等待队列中的后继节点共用同一个字段
        ...
        try {
            ...
            for (;;) {
                final AbstractQueuedSynchronizer.Node prev = node.predecessor();
                //如果当前节点的前驱节点 == head,并且tryAcquireShared>=0
                if (prev == head) {
